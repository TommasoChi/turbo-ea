...
export interface DiagramInputs {
  ...
  /** Cards the view proposes (no inventory id): drawn as plain shapes, never as live cards. */
...
export function collectDiagramInputs(
  ...
  for (const n of live) {
    if (n.type === "ldvNode") {
      ...
      if (d.proposed || d.changeKind === "added") {
        // Proposed cards have no inventory id, so they cannot be live cards.
        freeforms.push({
          id: n.id,
          name: d.name,
          type: d.typeLabel || d.typeKey,
          changeKind: "NEW",
          color: d.typeColor,
          x: p.x,
          y: p.y,
      ...
      cards.push({
        ...
        marker:
          d.changeKind === "modified" ? "MODIFY" : d.changeKind === "removed" ? "REMOVE" : undefined,
      });
      exported.add(n.id);
  ...
  return { cards, rels, layers, groups, connectors, freeforms };
}
