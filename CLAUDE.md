...
- **SDK surface only.** Backend extensions may import exclusively from `app.services.extensions.sdk`. Backend SDK `1.2` uses exact `major.minor` declarations: the major must match and the extension minor must be less than or equal to the core minor; SDK 1.0/1.1 extensions therefore remain compatible. SDK 1.1 route-dependency re-exports remain supported. New SDK 1.2 routes should use `extension_context(key)`, which exposes the authenticated actor plus `core_query` (permission-shaped, read-only Card access), `audit` (standard EventBus with optional generic `entity_type/entity_id`), `resources` (standard links/files owned by a Card or one generic entity) and `notifications` (standard in-app delivery). Bridge writes share the request `AsyncSession`, never commit, and do not expose native Card/relation/Risk/Objective/Initiative writes. Guard tests: `tests/services/test_extension_sdk.py`, `test_extension_sdk_bridges.py`, `test_alembic_128_extension_sdk_bridges.py`. Extension tables are `ext_{key}_*` (tracked in `extension_schema_versions`, never in core Alembic), permission keys `ext.{key}.*` (namespace enforced by the loader), API routes mounted under `/api/v1/ext/{key}/` at import time in `app.main` (**backend**-code install/uninstall ⇒ `needs_restart`; UI-only/content/metamodel bundles install live — `RUNTIME_CAPABILITIES` in `installer.py` is `{"backend"}` only, since `/ext-assets` serves from disk per request and `/extensions/ui-manifest` refreshes from the DB per call).
